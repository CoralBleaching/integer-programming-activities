...
|`operator<<(std::ostream &os, const Container<valueType> &vector)`|Overloads the `<<` operator to allow printing of containers. |
...
|Function|Description|
|---|---|
| `preprocessingStep1(Matrix &A, std::set<size_t> &keptVariables)` | The first step of pre-processing. This function calls `isUnitVector` to identify unit vector rows and removes them as restrictions, identifying variables associated with those rows (elements) and removing them. It then looks for further restrictions associated with those variables and also removes them. The indices of remaining variables are stored in `keptVariables`. It returns the indices of columns removed (the associated variables have been selected). |
...
|`preprocessingStep3(Matrix &A, std::set<size_t> &keptVariables)`| Calls `preprocessingSubRoutine` with `LineWise` set to `COLUMN_WISE` to remove unnecessary columns. The `keptVariables` parameter is passed to `preprocessingSubRoutine` to keep track of which columns have not been removed. Returns the indices of columns that have been removed.|
